...
* 除了Cookie或裝置ID所追蹤的使用者外，某些廠商的第三方區段可能會自動包含通用ID。 例如，來自的區段 [!DNL Eyeota] 可能會自動包含ID5 ID，以及以下專案的區段： [!DNL Lotame] 可能包含UID2.0 ID。 區段詳細資訊包括每種型別的大小。 每個區段的一般使用費（列在區段名稱旁）適用；ID5 ID不需額外付費。
