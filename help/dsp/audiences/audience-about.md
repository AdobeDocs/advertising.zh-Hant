...
* DSP可使用[來源功能](/help/dsp/audiences/sources/source-about.md)，將第一方資料區段從支援的客戶資料平台轉譯為具有通用ID的區段。

* 澳洲廣告商可使用[來源功能](/help/dsp/audiences/sources/source-about.md)匯入[!DNL AdFixus]第一方區段，而不需將您的[!DNL AdFixus]通用ID轉譯為其他ID型別。

* DSP是[!DNL LiveRamp]的目的地平台，因此您可以[手動將已驗證的 [!DNL LiveRamp] [!DNL RampID]區段直接傳送至DSP](/help/dsp/audiences/sources/source-import-liveramp-segments.md)。
...
* （僅針對美國的位置） [所有協力廠商資料區段都可供30個以上提供者的DSP客戶使用](/help/dsp/audiences/third-party-data-providers.md)，包括[!DNL eXelate]、([!DNL Eyeota])、([!DNL LiveRamp])、[!DNL Lotame]、[!DNL TransUnion]等。
...
在[!UICONTROL Sources]檢視中，您可以設定來自支援客戶資料平台的第一方區段的來源，以將其轉換為包含指定通用ID型別的區段。 您也可以設定[!UICONTROL AdFixus ID]來源以匯入具有[!DNL AdFixus]個通用ID的區段（僅限澳洲）。 來源設定包含要與您的CDP或[!DNL AdFixus]團隊共用的自動產生來源金鑰。

如需關於支援平台、支援的通用ID型別和設定工作流程的詳細資訊，請參閱[關於第一方對象來源](/help/dsp/audiences/sources/source-about.md)。

透過[!UICONTROL Sources]匯入的區段可用於可重複使用的對象，以及在適當位置設定中的[!UICONTROL Universal ID]目標定位。
