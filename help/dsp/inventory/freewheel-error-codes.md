...
| [!DNL Invalid external_deal_id:] \&lt;deal_id\> | 提交的交易ID不存在，或在Adobe端未啟用。 | 確定交易有效，然後重新提交廣告。 |
...
| DATA_DEAL_GET_FAILURE | 422 | 找不到交易或其設定有問題。 檢查錯誤訊息以取得詳細資料。 |
...
| DATA_DSP_GET_FAILURE | 422 | DSP不存在於系統中。 |
