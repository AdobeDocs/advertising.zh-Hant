...
失敗廣告提交的錯誤消息可來自廣告DSP或來自 [!DNL FreeWheel]。 錯誤消息顯示在 [!UICONTROL API Response] 列 [[!UICONTROL Freewheel Status] 對話](freewheel-check-status.md)。
...
| 代碼 | 意義 | 說明 | 後續步驟 |
|--- |--- |--- |--- |
| 401 | 未授權 | 訪問憑據不正確、缺失或無效。 | 聯繫您的Adobe帳戶團隊。 |
| 403 | 禁止 | 伺服器理解請求，但拒絕授權。 | 聯繫您的Adobe帳戶團隊。 |
| 404 | 未找到 | 您請求的資源不可用。 如果在PUT操作中找不到Creative ID ，則返回404。 | 聯繫您的Adobe帳戶團隊。 |
| 405 | 不允許使用方法 | 使用該資源不支援的請求方法對資源發出請求(例如，對需要通過POST發送資料的方法使用GET，或對只讀資源使用PUT)。 | 聯繫您的Adobe帳戶團隊。 |
| 408 | 請求超時 | 處理此請求時發生超時。 超時通常由對特定資源的獨佔訪問的併發請求引起。 | 收到此狀態後重新提交請求。 如果問題仍然存在，請與您的Adobe帳戶團隊聯繫。 |
| 422 | 不可處理實體 | 資源無效。 當請求正文無效或建立/更新的資源無效（例如，如果找不到交易ID）時，會發生此錯誤。 請參閱 [FreeWheel API 422錯誤](#freewheel-422-errors) 的子菜單。 | 聯繫您的Adobe帳戶團隊。 |
| 500 | 內部伺服器錯誤 | API系統錯誤。 | 聯繫您的Adobe帳戶團隊。 |
