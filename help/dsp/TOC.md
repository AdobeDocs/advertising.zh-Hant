---
type: Documentation
role: User, Admin
cloud: Experience Cloud
solution: Advertising
product: advertising cloud
user-guide-title: Advertising DSP 指南
user-guide-description: 了解使用 Advertising DSP 管理、最佳化和報告展示廣告策略所需的主要功能、工作、設定和其他資源。
breadcrumb-title: DSP 指南
index: true
source-git-commit: ce26d622bde31f289466c581a85724b93e4eae62
workflow-type: tm+mt
source-wordcount: '887'
ht-degree: 6%

---


# Advertising DSP 指南 {#dsp}

<!-- add back to about below "Edit the Advertisers with Access to a Deal ID" when reintroduced:
      + [Edit the Attribute Tags for a Deal ID](/help/dsp/inventory/deal-id-edit-attribute-tags.md)
-->

<!-- add back to Inventory > SAS section when reimplemented:
      +  [View Event-Tracking Pixels for a [!UICONTROL Simple Ad Serving] Deal](/help/dsp/inventory/simple-deal-show-pixels.md)
-->

+ [新增功能](/help/dsp/home.md)
+ 導言 {#introduction}
   + [Adobe廣DSP告](/help/dsp/introduction/dsp-about.md)
   + 關鍵功能 {#features}
      + [優化](/help/dsp/introduction/features/optimization.md)
      + [品牌安全與媒體質量](/help/dsp/introduction/features/brand-safety-media-quality.md)
      + [跨設備解決方案](/help/dsp/introduction/features/cross-device-solutions.md)
   + [帳戶資金](/help/dsp/introduction/billing/account-funding.md)
   + [支援的貨幣](/help/dsp/currency.md)
   + [支援消費者隱私權](https://experienceleague.adobe.com/docs/advertising/privacy/home.html)<!-- This is a duplicate link to this file, so using an absolute link here instead of a relative link. Github doesn't allow duplicate links via relative links. -->
   + [原則和指導方針](https://experienceleague.adobe.com/docs/advertising/privacy/home.html)<!-- This is a duplicate link to this file, so using an absolute link here instead of a relative link. Github doesn't allow duplicate links via relative links. -->
   + [高級測量解決方案](/help/dsp/introduction/advanced-measurement-services.md)
+ Campaign Management {#campaign-management}
   + [Campaign Management結構概述](/help/dsp/campaign-management/campaign-management-overview.md)
   + 行銷活動 {#campaigns}
      + [關於Campaign Management](/help/dsp/campaign-management/campaigns/campaign-about.md)
      + [建立市場活動](/help/dsp/campaign-management/campaigns/campaign-create.md)
      + [編輯市場活動](/help/dsp/campaign-management/campaigns/campaign-edit.md)
      + [複製市場活動](/help/dsp/campaign-management/campaigns/campaign-duplicate.md)
      + [查看市場活動的詳細報表](/help/dsp/campaign-management/campaigns/campaign-view-report.md)
      + [存檔或取消存檔市場活動](/help/dsp/campaign-management/campaigns/campaign-archive-unarchive.md)
      + [查看市場活動的更改日誌](/help/dsp/campaign-management/campaigns/campaign-change-log.md)
      + [市場活動設定](/help/dsp/campaign-management/campaigns/campaign-settings.md)
      + [市場活動啟動核對表](/help/dsp/campaign-management/campaign-launch-checklist.md)
   + 包 {#packages}
      + [關於包管理](/help/dsp/campaign-management/packages/package-about.md)
      + [建立包](/help/dsp/campaign-management/packages/package-create.md)
      + [編輯包](/help/dsp/campaign-management/packages/package-edit.md)
      + [複製包](/help/dsp/campaign-management/packages/package-duplicate.md)
      + [將放置附加到包](/help/dsp/campaign-management/packages/package-attach-placement.md)
      + [存檔或取消存檔包](/help/dsp/campaign-management/packages/package-archive-unarchive.md)
      + [查看包的更改日誌](/help/dsp/campaign-management/packages/package-change-log.md)
      + [包設定](/help/dsp/campaign-management/packages/package-settings.md)
   + 放置 {#placements}
      + [關於放置管理](/help/dsp/campaign-management/placements/placement-about.md)
      + [建立放置](/help/dsp/campaign-management/placements/placement-create.md)
      + [編輯放置](/help/dsp/campaign-management/placements/placement-edit.md)
      + [快速編輯密鑰放置設定](/help/dsp/campaign-management/placements/placement-quick-edit.md)
      + [複製放置](/help/dsp/campaign-management/placements/placement-duplicate.md)
      + [將廣告附加到放置](/help/dsp/campaign-management/ads/ad-attach-to-placement.md)
      + [編輯放置廣告計畫](/help/dsp/campaign-management/placements/placement-edit-ad-schedule.md)
      + [查看放置的詳細報告](/help/dsp/campaign-management/placements/placement-view-report.md)
      + [查看放置的更改日誌](/help/dsp/campaign-management/placements/placement-change-log.md)
      + [暫停或激活放置](/help/dsp/campaign-management/placements/placement-pause-activate.md)
      + [存檔或取消存檔位置](/help/dsp/campaign-management/placements/placement-archive-unarchive.md)
      + [放置設定](/help/dsp/campaign-management/placements/placement-settings.md)
   + 廣告 {#ads}
      + [關於廣告管理](/help/dsp/campaign-management/ads/ad-about.md)
      + [認證廣告服務合作夥伴](/help/dsp/campaign-management/ads/certified-ad-servers.md)
      + [建立單個廣告](/help/dsp/campaign-management/ads/ad-create.md)
      + [建立多個第三方廣告](/help/dsp/campaign-management/ads/ad-create-multiple.md)
      + [建立廣告 [!DNL Flashtalking]](/help/dsp/campaign-management/ads/ad-create-flashtalking.md)
      + [編輯廣告](/help/dsp/campaign-management/ads/ad-edit.md)
      + [複製廣告](/help/dsp/campaign-management/ads/ad-duplicate.md)
      + [將廣告附加到放置](https://experienceleague.adobe.com/docs/advertising/dsp/campaign-management/placements/ad-attach-to-placement.html)<!-- Dupe link to this file (in Placements chapter), so need to use an absolute link in one of the listings -->
      + [列出與廣告關聯的放置](/help/dsp/campaign-management/ads/ad-list-placements.md)
      + [查看廣告的詳細報告](/help/dsp/campaign-management/ads/ad-view-report.md)
      + [存檔或取消存檔廣告](/help/dsp/campaign-management/ads/ad-archive-unarchive.md)
      + 廣告設定 {#ad-settings}
         + [音頻廣告設定](/help/dsp/campaign-management/ads/ad-settings-audio.md)
         + [連接的電視設定](/help/dsp/campaign-management/ads/ad-settings-connected-tv.md)
         + [顯示廣告設定](/help/dsp/campaign-management/ads/ad-settings-display.md)
         + [移動廣告設定](/help/dsp/campaign-management/ads/ad-settings-mobile.md)
         + [本機顯示廣告設定](/help/dsp/campaign-management/ads/ad-settings-native.md)
         + [預滾動廣告設定](/help/dsp/campaign-management/ads/ad-settings-pre-roll.md)
         + [通用視頻廣告設定](/help/dsp/campaign-management/ads/ad-settings-universal-video.md)
      + [支援的廣告類型的規範](/help/dsp/campaign-management/ads/ad-specs.md)
   + 市場活動報告 {#campaign-reports}
      + [關於平台內報告](/help/dsp/campaign-management/reports/campaign-reports-about.md)
      + [從Campaign Management視圖導出資料](/help/dsp/campaign-management/reports/campaign-export-data.md)
      + [查看位置的站點、廣告和頻率詳細資訊](/help/dsp/campaign-management/reports/placement-details-view.md)
      + [查看放置診斷報告](/help/dsp/campaign-management/reports/placement-diagnostics.md)
      + 管理資料視圖 {#data-views}
         + [關於市場活動資料視圖](/help/dsp/campaign-management/reports/campaign-data-views-about.md)
         + [更改列視圖](/help/dsp/campaign-management/reports/column-view-change.md)
         + [建立自定義列視圖](/help/dsp/campaign-management/reports/column-view-create.md)
         + [編輯自定義列視圖](/help/dsp/campaign-management/reports/column-view-edit.md)
         + [篩選市場活動資料](/help/dsp/campaign-management/reports/campaign-data-filter.md)
         + [對列排序](/help/dsp/campaign-management/reports/campaign-data-sort.md)
      + [管理資料可視化](/help/dsp/campaign-management/reports/campaign-data-visualization-manage.md)
   + 使用電子錶格正確放置設定 {#qa-spreadsheet}
      + [關於使用電子錶格更正放置設定](/help/dsp/campaign-management/qa/qa-about.md)
      + [下載市場活動的位置設定](/help/dsp/campaign-management/qa/qa-sheet-download.md)
      + [上載市場活動的位置設定](/help/dsp/campaign-management/qa/qa-sheet-upload.md)
      + [已下載/已上載電子錶格中的列](/help/dsp/campaign-management/qa/qa-sheet-columns.md)
   + [宏DSP](/help/dsp/campaign-management/macros.md)
   + [鍵盤快捷鍵](/help/dsp/campaign-management/reports/keyboard-shortcuts.md)
   + [關於Campaign Management的常見問題](/help/dsp/campaign-management/faq-campaign-management.md)
   + [關於通用視頻的常見問題](/help/dsp/campaign-management/faq-universal-video.md)
+ 優化 {#optimization}
   + [如何 [!DNL DSP] 優化您的市場活動](/help/dsp/optimization/optimization-how-dsp-optimizes-campaigns.md)
   + [優化目標及其使用方法](/help/dsp/optimization/optimization-goals.md)
   + [位置級預標過濾器及其使用方法](/help/dsp/optimization/optimization-pre-bid-filters.md)
   + 自定義目標 {#custom-goals}
      + [關於自定義目標](/help/dsp/optimization/custom-goal-about.md)
      + [建立自定義目標](/help/dsp/optimization/custom-goal-create.md)
      + [構建自定義目標的最佳做法](/help/dsp/optimization/custom-goal-best-practices.md)
   + [設定績效活動的最佳做法](/help/dsp/optimization/campaign-best-practices-performance.md)
   + [排除效能故障](/help/dsp/optimization/troubleshooting-performance.md)
+ 對象 {#audiences}
   + [關於受眾管理](/help/dsp/audiences/audience-about.md)
   + 區段 {#segments}
      + [建立和實施自定義段](/help/dsp/audiences/custom-segment-create.md)
      + [!UICONTROL CCPA Opt-out-of-Sale] 區段 {#ccpa-opt-out-of-sale-segments}
         + [關於 [!UICONTROL CCPA Opt-out-of-Sale] 段](/help/dsp/audiences/ccpa-opt-out-about.md)
         + [建立和實施 [!UICONTROL CCPA Opt-out-of-Sale] 段](/help/dsp/audiences/ccpa-opt-out-segment-create.md)
         + [檢索 [!UICONTROL Consumer Opt-Out-of-Sale] 報告](/help/dsp/audiences/ccpa-opt-out-segment-report-retrieve.md)
      + [編輯段資訊](/help/dsp/audiences/segment-edit.md)
      + [共用或停止共用段](/help/dsp/audiences/segment-share.md)
      + [查看段的跟蹤像素](/help/dsp/audiences/segment-view-pixels.md)
      + [刪除段](/help/dsp/audiences/segment-delete.md)
   + 對象 {#reusable-audiences}
      + [建立可重用的受眾](/help/dsp/audiences/reusable-audience-create.md)
      + [複製可重複使用的受眾](/help/dsp/audiences/reusable-audience-duplicate.md)
      + [編輯可重用受眾](/help/dsp/audiences/reusable-audience-edit.md)
      + [查看有關可重用受眾的詳細資訊](/help/dsp/audiences/reusable-audience-view-details.md)
      + [共用可重複使用的受眾](/help/dsp/audiences/reusable-audience-share.md)
      + [導出可重複使用的受眾](/help/dsp/audiences/reusable-audience-export.md)
      + [將可重用受眾的段鍵複製到剪貼簿](/help/dsp/audiences/reusable-audience-clipboard.md)
      + [刪除可重用受眾](/help/dsp/audiences/reusable-audience-delete.md)
      + [受眾設定](/help/dsp/audiences/audience-settings.md)
      + [受眾段邏輯的語法](/help/dsp/audiences/audience-segment-logic-syntax.md)
   + 源 {#sources}
      + [關於從受眾源激活經過驗證的段](/help/dsp/audiences/sources/source-about.md)
      + [從持久ID夥伴激活經過驗證的段](/help/dsp/audiences/sources/source-durable-id.md)
      + [建立受眾源以激活第一方受眾](/help/dsp/audiences/sources/source-create.md)
      + [受眾源設定](/help/dsp/audiences/sources/source-settings.md)
   + [可用的第三方資料提供程式](/help/dsp/audiences/third-party-data-providers.md)
+ 庫存 {#inventory}
   + [庫存功能概覽](/help/dsp/inventory/inventory-overview.md)
   + [使用 [!DNL Roku Inventory]](/help/dsp/inventory/roku-inventory.md)
   + [SSP合作夥伴](/help/dsp/inventory/ssp-partners.md)
   + [!DNL On Demand] 庫存 {#on-demand}
      + [關於 [!DNL On Demand] 高級庫存](/help/dsp/inventory/on-demand-inventory-about.md)
      + 可用 [!DNL On Demand] 高級庫存發佈器 {#publishers}
         + [[!DNL On Demand] 亞太地區高級庫存出版商](/help/dsp/inventory/on-demand-inventory-publishers-apac.md)
         + [[!DNL On Demand] 澳大利亞和紐西蘭的高級庫存出版商](/help/dsp/inventory/on-demand-inventory-publishers-anz.md)
         + [[!DNL On Demand] 歐洲、中東和非洲的高級庫存出版商](/help/dsp/inventory/on-demand-inventory-publishers-emea.md)
         + [[!DNL On Demand] 北美的高級庫存出版商](/help/dsp/inventory/on-demand-inventory-publishers-na.md)
      + [訂閱和請求訪問 [!DNL On Demand] 高級庫存交易](/help/dsp/inventory/on-demand-inventory-subscribe.md)
      + [重新請求 [!DNL On Demand] 高級庫存交易](/help/dsp/inventory/on-demand-inventory-rerequest.md)
      + [查看您的狀態 [!DNL On Demand] 交易請求和訂閱](/help/dsp/inventory/on-demand-inventory-view-status.md)
   + 專用清單 {#private-inventory}
      + [關於專用清單](/help/dsp/inventory/private-inventory-about.md)
      + 交易ID {#deal-ids}
         + [!UICONTROL Deal ID Inbox] {#inbox}
            + [關於 [!UICONTROL Deal ID Inbox]](/help/dsp/inventory/deal-id-inbox-about.md)
            + [在中接受交易 [!UICONTROL Deal ID Inbox]](/help/dsp/inventory/deal-id-inbox-accept.md)
         + 手動輸入的交易ID {#manual}
            + [手動建立 [!UICONTROL Deal ID] 詳細資訊](/help/dsp/inventory/deal-id-create.md)
            + [手動 [!UICONTROL Deal ID] 設定](/help/dsp/inventory/deal-id-settings.md)
         + 計畫擔保交易 {#programmatic-guaranteed-deal}
            + [計畫擔保交易概述](/help/dsp/inventory/programmatic-guaranteed-about.md)
            + [計畫保證交易談判技巧](/help/dsp/inventory/programmatic-guaranteed-tips.md)
            + [設定計畫保證交易](/help/dsp/inventory/programmatic-guaranteed-set-up.md)
            + [!DNL FreeWheel] 計畫擔保交易 {#freewheel}
               + [中計畫保證交易設定概述 [!DNL FreeWheel]](/help/dsp/inventory/freewheel-overview.md)
               + [提交計畫保證交易的廣告以 [!DNL FreeWheel]](/help/dsp/inventory/freewheel-submit.md)
               + [檢查廣告狀態 [!DNL FreeWheel] 計畫擔保交易](/help/dsp/inventory/freewheel-check-status.md)
               + [FreeWheel廣告提交的錯誤代碼](/help/dsp/inventory/freewheel-error-codes.md)
      + [編輯私有交易設定](/help/dsp/inventory/deal-id-edit.md)
      + [指定私人交易的放置和廣告](/help/dsp/inventory/deal-id-attach-placements.md)
      + [編輯具有私人交易訪問權限的帳戶](/help/dsp/inventory/deal-id-share.md)
      + [編輯可訪問私人交易的廣告商](/help/dsp/inventory/deal-id-edit-advertisers.md)
      + [查看私人交易的拍賣洞察](/help/dsp/inventory/private-deal-auction-insights.md)
      + [列出私人交易的定位和廣告](/help/dsp/inventory/private-deal-view-placements.md)
      + [存檔或取消存檔私有交易](/help/dsp/inventory/private-deal-archive-unarchive.md)
   + [!UICONTROL Simple Ad Serving] 交易 {#sas}
      + [關於 [!UICONTROL Simple Ad Serving]](/help/dsp/inventory/simple-deal-about.md)
      + [建立 [!UICONTROL Simple Ad Serving] 交易](/help/dsp/inventory/simple-deal-create.md)
      + [編輯 [!UICONTROL Simple Ad Serving] 交易設定](/help/dsp/inventory/simple-deal-edit.md)
      + [[!UICONTROL Simple Ad Serving] 交易設定](/help/dsp/inventory/simple-deal-settings.md)
   + [查看交易的詳細報表](/help/dsp/inventory/deal-view-report.md)
+ 自定義報告 {#custom-reports}
   + [關於自定義報告](/help/dsp/reports/report-about.md)
   + [建立自定義報告](/help/dsp/reports/report-create.md)
   + [複製自定義報告](/help/dsp/reports/report-copy.md)
   + [編輯自定義報告](/help/dsp/reports/report-edit.md)
   + [刪除自定義報告](/help/dsp/reports/report-delete.md)
   + [運行自定義報告](/help/dsp/reports/report-run-now.md)
   + [自定義報表設定](/help/dsp/reports/report-settings.md)
   + [可用報表列](/help/dsp/reports/report-columns.md)
   + [關於 [!UICONTROL Household] 報告](/help/dsp/reports/faq-household-report.md)
   + 報告目標 {#report-destinations}
      + [關於 [!UICONTROL Report Destinations]](/help/dsp/reports/report-destinations/report-destination-about.md)
      + [建立 [!UICONTROL Report Destination]](/help/dsp/reports/report-destinations/report-destination-create.md)
      + [編輯 [!UICONTROL Report Destination]](/help/dsp/reports/report-destinations/report-destination-edit.md)
      + [刪除 [!UICONTROL Report Destination]](/help/dsp/reports/report-destinations/report-destination-delete.md)
      + [[!UICONTROL Report Destination] 設定](/help/dsp/reports/report-destinations/report-destination-settings.md)
+ 管理 {#admin}
   + [編輯您的配置檔案](/help/dsp/admin/user-own-profile-edit.md)
   + 用戶 {#users}
      + [邀請新用戶](/help/dsp/admin/user-invite.md)
      + [重新發送掛起的用戶邀請](/help/dsp/admin/user-resend-invite.md)
      + [取消邀請掛起的用戶](/help/dsp/admin/user-uninvite.md)
   + 阻止的站點 {#sites}
      + [關於帳戶級和廣告商級被阻止的站點清單](/help/dsp/admin/blocked-sites-list-about.md)
      + [編輯帳戶級或廣告商級被阻止站點清單](/help/dsp/admin/blocked-sites-list-edit.md)
   + 通知 {#notifications}
      + [關於警報訂閱](/help/dsp/admin/alerts-about.md)
      + [管理警報訂閱](/help/dsp/admin/alerts-manage.md)
+ [Adobe Experience Cloud 的整合](https://experienceleague.adobe.com/docs/advertising/integrations/home.html)<!-- This is a duplicate link to this file, so using an absolute link here instead of a relative link. Github doesn't allow duplicate links via relative links. -->
+ [客戶支援](https://helpx.adobe.com/tw/contact/enterprise-support.ec.html)
+ [Adobe Experience Cloud 發行說明](https://experienceleague.adobe.com/docs/release-notes/experience-cloud/current.html)
