...
**[!UICONTROL Environment]:** （僅限通用視頻廣告格式）要作為目標包括在放置中的設備環境（案頭、移動、連接的電視）。 至少指定一個。

在自定義報告中，放置級維「設備環境」指示目標環境。
...
**[!UICONTROL Flight pacing]:** （僅包含放置級別調節的放置）如何調整和交付：
...
**[!UICONTROL Intraday pacing]:** （僅限放置級調音的放置）如何在飛行中每天間調整和交付：

* *[!UICONTROL Even]:* （預設）根據庫存可用性調整交貨量。 一般來說，在拍賣量較大時，白天每小時投放的廣告較多，而早晚投放的廣告較少。

* *[!UICONTROL ASAP]:* （預設）將交付速度加快到 *甚至*。
...
* [!UICONTROL Public]:（除Roku外的所有放置類型）具有訪問權限的所有未結交DSP換庫存。 您可以包括和排除公用清單。
...
* [!UICONTROL On Demand] | [!UICONTROL Roku On Demand]:全部 [溢價，無擔保 [!UICONTROL On Demand] 庫存](/help/dsp/inventory/on-demand-inventory-about.md) 或 [!UICONTROL On Demand] [!DNL Roku] 交易 [!DNL Roku] 配售) [!DNL DSP]。 可以包括和排除 [!UICONTROL On Demand] 庫存。
...
* 要瞄準庫存類型：
   ...
   1. （可選）更改源以包括：
      ...
      1. 指定要包括和排除的源：
         ...
         * 排除 [!UICONTROL Public] 或 [!UICONTROL On source]按一下 **[!UICONTROL Exclude]** 的子菜單。
...
**[!UICONTROL Site Categories]:** (可選；可用 **[!UICONTROL Paste list of targeted sites]** 是 *[!UICONTROL Off]*)所選站點層中的站點類別，以包括或排除（但不包括兩者）作為目標。 從基於站點主題映射DSP的垂直站點清單中進行選擇：
...
>[!NOTE]
>
>* 除了應用客戶級和廣告商級被阻止的站點清單外，還應用了客戶級和廣告商級被阻止的站DSP點清單 [全局阻止的站點清單](/help/dsp/introduction/features/brand-safety-media-quality.md)包括廣告認為不安全的網站。
...
**[!UICONTROL Placement Cap]:** （可選）唯一設備或人員的次數(取決於指定的 [!UICONTROL Cross Device Level] 廣告)。 選項包括 *[!UICONTROL Unlimited]* 或每天、每週或每月的特定金額。
...
**[!UICONTROL Topic Targeting]:** (可選；可用於配置有 [!DNL Comscore] 和 [!DNL Grapeshot] 段)特定段名稱或ID [!DNL Comscore] 和 [!DNL Grapeshot] 作為目標。 此功能可能需要額外費用。 要激活此功能並設定主題段，請與Adobe帳戶團隊聯繫。
