...
檢視「[位置層級競標前篩選器」上每個競標前篩選器的說明，以及如何使用](/help/dsp/optimization/optimization-pre-bid-filters.md)。
...
**[!UICONTROL Optimization Goal]：**&#x200B;封裝的最佳化目標。 請參閱「[最佳化目標」中每個最佳化目標的說明，以及如何使用它們](/help/dsp/optimization/optimization-goals.md)」。
...
* [!UICONTROL Private] | [!UICONTROL Roku Private]：您與在DSP中設定的發行者之間的現有私人交易（或[!DNL Roku]個位置現有的私人[!DNL Roku]交易），以及您現有的[私人交易清單](/help/dsp/inventory/lists-deals-manage.md)。 您可以包含（但不排除）公用詳細目錄。

  從[!UICONTROL Deals]索引標籤，您可以依關鍵字、金鑰、交易ID或自訂標籤來搜尋清單。 從[!UICONTROL Deal Lists]索引標籤，您可以依交易清單名稱或交易清單ID搜尋清單。

* [!UICONTROL On Demand] | [!UICONTROL Roku On Demand]：您已在[!DNL DSP]中訂閱的所有[進階、不保證的[!UICONTROL On Demand]詳細目錄](/help/dsp/inventory/on-demand-inventory-about.md) （或[!DNL Roku]個位置的[!UICONTROL On Demand] [!DNL Roku]交易）。 您可以包含和排除[!UICONTROL On Demand]詳細目錄。
...
* *[!UICONTROL Universal ID]*：鎖定以使用者隱私權為主的ID；選取一種ID型別。 可用的選項由[!UICONTROL Geo-Targeting]區段中選取的地理目標決定。 搭配直接匯入至DSP[&#128279;](/help/dsp/audiences/sources/source-import-liveramp-segments.md)的[!DNL RampID] 區段、DSP將您的PII轉換為通用ID的[區段](/help/dsp/audiences/sources/source-about.md)、串流至DSP的[第一方 [!DNL AdFixus] 區段](/help/dsp/audiences/sources/source-adfixus.md)或追蹤通用ID的[自訂區段](/help/dsp/audiences/custom-segment-create.md)使用。

   * *[!UICONTROL AdFixus]*：目標[!DNL AdFixus] ID已匯入至Advertising DSP。
  ...
  **[!UICONTROL Terms of service]**：使用通用ID的服務合約條款。 您或DSP帳戶中的其他使用者必須接受條款一次，才能匯入ID、將資料轉換為新ID型別或目標ID型別。 若客戶擁有受管理的服務合約，您的Adobe客戶團隊將代表貴組織取得您的同意並接受條款。 若要閱讀條款，請按一下&#x200B;**>**。 若要接受條款，請捲動至條款底部，然後按一下&#x200B;**[!UICONTROL Accept]**。
...
**[!UICONTROL DoubleVerify ABS segment ID]：** （選擇性；[!DNL DoubleVerify]客戶僅限；僅適用於案頭前段、標準及點播顯示，以及原生顯示和視訊位置；不支援交易的[預設程式化預留位置](/help/dsp/inventory/programmatic-guaranteed-about.md)） [!DNL DoubleVerify Authentic Brand Suitability]區段ID與組織的[!DNL DoubleVerify]帳戶相關聯，以用於位置。 指定ID會使用針對指定區段ID設定的自訂品牌安全規則，封鎖出價後的曝光數。 DSP會為帳戶開立帳單，以取得區段ID的使用量。
...
>[!NOTE]
>
>（[!DNL Roku]位置） [!DNL Roku]核准的第三方追蹤廠商包括[!DNL Acxiom]、[!DNL Comscore]、[!DNL Data Plus Math]、[!DNL Experian]、[!DNL Factual]、[!DNL Kantar]、[!DNL Marketing Evolution]、[!DNL Nielsen]、[!DNL Nielsen Catalina Solutions]、[!DNL NinthDecimal]、[!DNL Oracle]、[!DNL Placed]、[!DNL Polk]和[!DNL Research Now]以及[!DNL TransUnion]。
