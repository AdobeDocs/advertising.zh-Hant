...
當您鎖定只嚴格要求VAST格式的連線電視或詳細目錄時（通常來自Google Ad Manager、Appnexus、SpotX和FreeWheel等供應來源），使用[!UICONTROL VAST]不允許可檢視度測量。 也請將此選項用於先前與標準前段廣告(VAST)或手機+平板電腦標準前段廣告(VAST)版位/廣告相容的詳細目錄。
