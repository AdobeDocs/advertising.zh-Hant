...
廣DSP告提供一套品牌保護功能，以確保您的每項活動都能在品牌安全的環境中接觸到真正的用戶。
...
除了提供中央質量管理，我們還相信讓廣告商設計與其品牌相一致的控制。 DSP提供整合 [!DNL Comscore]。 [!DNL DoubleVerify]。 [!DNL Integral Ad Science]。 [!DNL Oracle Data Cloud], [!DNL Peer39]確保每個廣告商都能夠選擇其所需的欺詐保護、上下文過濾和關鍵字目標。
...
* **站點分類：** 為確保輕鬆確定內容目標和阻止，我們會根據屬性的內DSP容為每個屬性添加定義的站點類別標籤。 你可以 [每個放置的目標或排除這些站點類別](/help/dsp/campaign-management/placements/placement-settings.md) 根據投放目標。

### 全面支援站點阻止

提DSP供全局阻止站點清單和為廣告商和帳戶建立自定義阻止站點清單的選項。

#### 全局阻DSP止的站點清單 {#global-blocked-sites}

維護DSP被全局阻止的站點清單，列出在上運行廣告時被認為不安全的站點。 此清單包含包含令人反感內容（如仇恨或恐怖）的站點，以及受bot 、假預滾、不匹配的域和其他欺詐活動感染的站點。

作為我們的品牌安全計畫的一部分，我們將根除欺詐廣告商的活動，使用圖表阻止站點清單中的度量來篩選所有站點。 所有未通過品牌安全檢查的站點都將添加到全局阻止的站點清單中。 因DSP為動態管理此清單，所以根據最新的品牌安全分析，站點可以隨時在清單上移動或移出。

當您將一個站點作為放置目標包含在全局阻止的站點清單中時，該站點將標籤為紅色感嘆號(!)。 這表示廣告不會在標籤的站點上運行。

>[!NOTE]
>
>通過啟用「」，您可以選擇繞過全局阻止站點清單，以獲取附加到可信私人交易的標準顯示廣告。[!UICONTROL Allow unscreened sites]選項 [放置設定](/help/dsp/campaign-management/placements/placement-settings.md)。 如有必要，Adobe帳戶團隊還可以在交易的發佈者設定中選擇禁用公共（拍賣級別）交易的站點阻止。
...
有關功能的詳細資訊，請直接與首選供應商聯繫，或與Adobe客戶團隊聯繫。
...
* 開始 [!DNL Oracle Data Cloud]，聯繫人 [!DNL Oracle Data Cloud] 或您的Adobe團隊。
...
有關功能的詳細資訊，請與 [!DNL DoubleVerify] 直接聯繫您的Adobe客戶團隊。
