...
>[!NOTE]
>
>您可以啟用[位置設定](/help/dsp/campaign-management/placements/placement-settings.md)中的[!UICONTROL Allow unscreened sites]選項，選擇略過附加至受信任私人交易的標準顯示廣告的全域封鎖網站清單。 如有必要，Adobe客戶團隊也可以選擇在交易的發佈者設定中，針對公開（拍賣層級）交易停用網站封鎖。
...
DSP已與[!DNL DoubleVerify]合作提供其[!DNL Authentic Brand Suitability]目標定位解決方案，可讓您建立一組集中的品牌適用性需求，以針對所有購買平台進行目標定位，確保一致性。

使用必要的目標定位建置[!DNL DoubleVerify]區段後，您就可以在DSP中使用它，跨網路環境使用競標前來複製您的競標後區塊規則。

您可以為每個廣告商指定[!DNL DoubleVerify]區段識別碼，然後選擇性地[為每個位置](/help/dsp/campaign-management/placements/placement-settings.md)啟用或停用[!UICONTROL Authentic Brand Suitability]。 DSP會為帳戶開立帳單，以取得區段ID的使用量。
