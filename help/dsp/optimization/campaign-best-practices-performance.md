...
可DSP以優化注重績效的營銷活動，以便以最低的每次收購成本(CPA)或最高的廣告支出回報(ROAS)進行投放。
...
* 如果廣告商是Adobe Analytics或Adobe Audience Manager的客戶，則您可以構建第一方網段（如首頁訪問者、產品頁面或購物車放棄者），並將它們用作中的放置目DSP標。
...
* **飛行起步和日內起步：** 對於這兩種類型的調步，選擇 *[!UICONTROL Even]* 通過在每天和整個飛行中均勻地起步，最大限度地實現效能目標。

   >[!CAUTION]
   >
   >使用 *[!UICONTROL FrontLoad]* 和 *[!UICONTROL Aggressive Front Load]* 飛行起步和 *[!UICONTROL ASAP]* 只有在您完全排定交付優先順序並花在效能優化上時，才會調整日內調整，因為這些策略可能會對您期望的效能KPI產生負面影響。
...
* **[!UICONTROL Device Targeting]**:
   ...
   * 不瞄準 [!UICONTROL Firefox] 和 [!UICONTROL Safari] 因為目標和測量的限制。 請聯繫您的Adobe客戶團隊以瞭解有關 [!DNL Adobe] 支援 [!DNL Safari ITP]。
