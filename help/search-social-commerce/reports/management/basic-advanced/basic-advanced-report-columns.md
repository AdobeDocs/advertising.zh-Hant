...
| [!UICONTROL Account Status] | Search、Social和Commerce中的帳戶狀態： <ul><li><i>[!UICONTROL Enabled]：</i> （預設）對於同步的廣告網路，Search、Social和Commerce可以登入廣告網路帳戶來擷取行銷活動資料，並啟用最佳化和追蹤產生等其他適用功能。<br><br>對於未同步的廣告網路，可運用如最佳化及/或追蹤產生等適用功能。</li><li><i>[!UICONTROL Disabled]：</i> 對於同步的廣告網路，搜尋、社交和商務不會登入廣告網路帳戶，因此不會擷取行銷活動資料，且最佳化和追蹤產生等其他適用功能會停用。 系統仍會儲存您啟用帳戶時收集的資料，但您日後建立的所有行銷活動管理檢視及所有報表，都不會包含帳戶停用期間的資料。<br><br>對於未同步的廣告網路，不提供適用的功能，例如最佳化和/或追蹤產生。</li></ul> |
...
| [!UICONTROL Avg Position] | 指定日期範圍內的廣告平均位置。<br><br>的 [!DNL Google Ads] 和 [!DNL Yahoo! Japan Ads] 促銷活動，此資料僅在2019年9月之前可用。 的 [!DNL Microsoft Advertising]，此資料僅截至2021年1月22日可用。 |
| [!UICONTROL Base URL] | 關鍵字的基本URL，包括為促銷活動或帳戶設定的任何附加引數。 其中不包含任何搜尋、社交和商務重新導向與追蹤程式碼。 |
...
| [!UICONTROL Content IS] | 您在顯示/對象網路上收到廣告的曝光數除以您符合資格接收的預估曝光數。 在 [!DNL Microsoft Advertising]，這稱為&quot;[!UICONTROL Audience IS].」 |
...
| [!UICONTROL Conversion Type] | ([!UICONTROL Transaction Report])轉換前的動作：<ul><li><i>[!UICONTROL Click:]</i> 轉換前至少發生一次付費點按。</li><li><i>[!UICONTROL Impression:]</i> 轉換前未發生任何付費點按，因此轉換是由檢視造成的（曝光時未進行任何付費點按）。</li></ul> |
...
| [!UICONTROL Currency] | 適用的貨幣型別（例如「USD」或「GBP」）。<br><br><b>注意：</b> 如果報表包含不同貨幣的帳戶資料，則任何一項「[!UICONTROL Total]「貨幣值只是欄中所有數字的總和，無論貨幣為何。 |
| [!UICONTROL Current Bid] | 目標的目前出價。 |
| [!UICONTROL Current First Page Bid] | ([!DNL Google Ads] 僅限行銷活動)在符合下列條件時，將廣告放置在搜尋結果第一頁所需的預估每次點按成本(CPC)競標： [!DNL Google] 搜尋查詢符合關鍵字。<br><br>對於單一關鍵字和相符型別組合，此值是該組合目前所需的第一個頁面競標。 當在多個行銷活動中使用相同的關鍵字和相符型別組合時，此值是目前所有執行個體中所需的最低第一頁競標。 |
...
| [!UICONTROL Custom Bid Level] | (僅以顯示網路為目標的Google行銷活動)投標的層級：依據 <i>[!UICONTROL Ad Group]</i>， <i>[!UICONTROL Age]</i>， <i>[!UICONTROL Gender]</i>， <i>[!UICONTROL Interest and List]</i>， <i>[!UICONTROL Keyword]</i>， <i>[!UICONTROL Placement]</i>， <i>[!UICONTROL Vertical]</i>， <i>[!UICONTROL None]</i>，或 <i>[!UICONTROL Unknown]</i>. |
| [!UICONTROL Description1] - [!UICONTROL Description4] | 廣告內文。 不同的創意型別有不同數量的必要和選用說明行。 若要檢視 [!UICONTROL Description3] 和 [!UICONTROL Description4] 欄於 [!DNL Microsoft Advertising] 回應式廣告或多媒體廣告，包含&quot;[!UICONTROL Descriptions]」欄。 |
| [!UICONTROL Descriptions] | ([!DNL Microsoft Advertising] 回應式廣告和多媒體廣告)為廣告的每個說明列新增一欄(&quot;[!UICONTROL Description1]「到」[!UICONTROL Description4]「)。 當您包含此欄時，就不需要包含其他欄 [!UICONTROL Description] 欄。 |
| [!UICONTROL Device] | ([!DNL Google Ads]， [!DNL Microsoft Advertising]， [!DNL Yahoo! Display Network]， [!DNL Yahoo! Japan Ads]、和 [!DNL Yahoo Native] 行銷活動)顯示廣告的裝置型別： <i>[!UICONTROL Computers]</i>， <i>[!UICONTROL Mobile]</i>， <i>[!UICONTROL Tablets]</i>， <i>[!UICONTROL Other]</i>，或 <i>[!UICONTROL N/A]</i> （沒有值）。 其他廣告網路的資料列值為N/A。<br><br>在搜尋行銷活動中，如果關鍵字、廣告和/或廣告擴充功能的追蹤範本或目的地URL包含依裝置追蹤資料的引數(`&ev_dvc={device}&ev_dvm={devicemodel}`)時，轉換資料也會包含在每種裝置型別的列中。 否則，如果轉換資料無法歸因於裝置型別，則會彙總在具有「」的單獨列中[!UICONTROL Device]的「 」值 [!UICONTROL N/A]. |
| [!UICONTROL Display Path 1] | ([!DNL Google Ads] 僅限延伸文字廣告)廣告的基礎URL之後的第一個顯示路徑。 |
| [!UICONTROL Display Path 2] | ([!DNL Google Ads] 僅限延伸文字廣告)廣告的基礎URL之後的第二個顯示路徑。 |
| [!UICONTROL Display Type] | 已過時 |
| [!UICONTROL Display URL] | 廣告的顯示URL，這是一般使用者在廣告中看到的內容。 |
| [!UICONTROL DMA] | ([!UICONTROL Geo Distribution Report]， [!UICONTROL Keyword Report])數值的指定市場區域，點選源自該區域（例如751代表丹佛）。 這是根據使用者的IP位址所決定。 |
...
| [!UICONTROL EF ID] | ([!UICONTROL Transaction Report]) (具有Adobe Advertising轉換追蹤服務和&quot;[!UICONTROL EF Redirect]&quot;包含權杖的追蹤方法)點選或轉換的權杖。<ul><li>的 [!DNL Google Ads] 搜尋廣告，EF ID為 `{gclid}:G:s`，其中包括Google點選ID (GCLID)和網路型別（「s」代表搜尋）。</li><li> 的 [!DNL Microsoft Advertising] 搜尋廣告，EF ID為 `{msclkid}:G:s`，其中包括Microsoft點選ID (MSCLKID)和網路型別（&quot;s&quot;以供搜尋）。</li><li>若是其他廣告網路上的搜尋廣告，EF ID包含瀏覽者ID、點選時間和網路型別。</li><li>若是顯示廣告，EF ID包含瀏覽者ID、點選或曝光時間，以及網路型別。</li></ul> |
...
| [!UICONTROL Link Type] | ([!UICONTROL Keyword Report]； [!DNL Google Ads] 和 [!DNL Microsoft Advertising] 僅限行銷活動；只有在為報告指定的歸因規則為「最後一個事件」時，資料才可用)當列報告由於點選廣告副檔名（而不是廣告本身）或產品/購物廣告而導致的轉換時，此欄會顯示被點選的連結型別和標題：<ul><li>`pla:*`  — 產品廣告列為 `pla:<product ID>`，例如「pla：8525822」。</li><li>`sl:*`  — 網站連結列為 `sl:<Sitelink text>`，例如「sl：請參閱目前的選件」。</li></ul> |
| [!UICONTROL Listing Match Type] | 關鍵字元合廣告清單的型別， <i>[!UICONTROL Content]</i> 針對以內容為目標的行銷活動中的廣告，或 <i>[!UICONTROL Sitecpc]</i> 適用於網站目標行銷活動中的版位。 的 [!DNL Microsoft Advertising] 關鍵字，其中可能包括多種相符型別(例如&quot;[!UICONTROL Broad]，[!UICONTROL Exact]「)。 |
| [!UICONTROL Location] | （顯示行銷活動）目標對象位置。 |
| [!UICONTROL Long Creative Title1] - [!UICONTROL Long Creative Title5] | （在已完成的報表列中） [!DNL Microsoft Advertising] 回應式及多媒體廣告)廣告的長標題。 若要檢視這些欄，請包含&quot;[!UICONTROL Long Creative Titles]」欄。 |
...
| [!UICONTROL Portfolio Spend Strategy] | ([!UICONTROL Portfolio Report])產品組合的支出策略： <i>[!UICONTROL Daily]</i>， <i>[!UICONTROL Weekly]</i>， <i>[!UICONTROL Monthly]</i>， <i>[!UICONTROL ROI]</i>， <i>[!UICONTROL Day of week]</i>， <i>[!UICONTROL Day of month]</i>， <i>[!UICONTROL CPT]</i>， <i>[!UICONTROL Marginal CPT]</i>， <i>[!UICONTROL Google Target CPA]</i>，或 <i>[!UICONTROL Google Target ROAS]</i>. |
| [!UICONTROL Portfolio Status] | 投資組合狀態：<ul><li><i>[!UICONTROL Optimize]：</i> 最佳化功能會收集相關行銷活動的點選和收入資料、模型化資料以最佳化出價，以及最佳化出價和/或行銷活動預算（取決於最佳化型別和行銷活動競標策略）。</li><li><i>[!UICONTROL Active]：</i> 最佳化功能會收集相關行銷活動的點選和收入資料，並將資料模型化，但不會最佳化競標或行銷活動預算。</li><li><i>[!UICONTROL Inactive]：</i> 最佳化功能會收集相關行銷活動的點選資料以用於報表用途，但不會模型化資料，也不會最佳化競標或行銷活動預算。</li></ul> |
...
| [!UICONTROL Search (Top) IS] | ([!DNL Google Ads] 和 [!DNL Microsoft Advertising])您從排名最前位置收到的曝光數（在有機搜尋結果上方），除以您有資格從排名最前位置收到的預估曝光數。 低於10%的百分比表示為「`<10%`「或」`0.0999`.」 |
...
| [!UICONTROL Search impr. share] | ([!DNL Google Ads] 僅限)您收到的曝光數除以您符合資格收到的預估曝光數。 10%以下的百分比會顯示為「&lt;10%」，而90%以上的百分比會顯示為「`>90%`.」 |
| [!UICONTROL Search lost abs. top IS (budget)] | ([!DNL Google Ads] 和 [!DNL Microsoft Advertising])您的廣告不是優先於自然搜尋結果之廣告的時間百分比，因為您的每日或每月預算太低。 針對Google廣告行銷活動，超過90%的百分比會表示為「`>90%`「或」`0.9001`.」 |
| [!UICONTROL Search lost abs. top IS (rank)] | ([!DNL Google Ads] 和 [!DNL Microsoft Advertising])由於廣告排名不佳，您的廣告不是第一個超過有機搜尋結果的時間百分比。 針對Google廣告行銷活動，超過90%的百分比會表示為「`>90%`「或」`0.9001`.」 |
| [!UICONTROL Search lost IS (budget)] | ([!DNL Google Ads] 僅限)因為每日或每月預算太低而未顯示廣告的時間百分比。 此量度僅在行銷活動層級提供。 超過90%的百分比表示為&quot;`>90%`「或」`0.9001`.」 |
| [!UICONTROL Search lost IS (rank)] | ([!DNL Google Ads] 僅限)由於廣告排名不佳而未顯示廣告的時間百分比。 超過90%的百分比表示為&quot;`>90%`「或」`0.9001`.」 |
| [!UICONTROL Search lost top IS (budget)] | ([!DNL Google Ads] 和 [!DNL Microsoft Advertising])您的廣告未顯示在有機搜尋結果上方的時間百分比，因為您的每日或每月預算太低。 的 [!DNL Google Ads] 行銷活動，超過90%的百分比顯示為&quot;`>90%`「或」`0.9001`.」 |
| [!UICONTROL Search lost top IS (rank)] | ([!DNL Google Ads] 和[！DNL [!DNL Microsoft Advertising]])由於廣告排名不佳而未顯示在有機搜尋結果上方的時間百分比。 針對Google廣告行銷活動，超過90%的百分比會表示為「`>90%`「或」`0.9001`.」 |
| [!UICONTROL Search Term] | ([!UICONTROL Transaction Report])使用者查詢的搜尋辭彙。 |
| [!UICONTROL SETrackingOnly] | 您是否正在追蹤帳戶但沒有投標： <i>[!UICONTROL TRUE]</i> 或 <i>[!UICONTROL FALSE]</i>. |
