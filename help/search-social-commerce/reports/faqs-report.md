...
| 原因 | 偵測/分析 | 解決方法 |
|---|---|---|
| 從廣告帳戶擷取點選資料的程式失敗。 | 沒有系統化的方式可偵測此問題，但您可能會注意到，即使廣告帳戶花費金錢，行銷活動仍不會顯示成本或點選資訊。 | 聯絡客戶支援： &lt;*您的搜尋、社交和商務使用者帳戶*>@support\.efrontier\.com.<!-- Escaped periods and using HTML code for angle brackets --><br><br>如果資料遺失超過24小時，則從成本預測中排除這些日期，直到擷取資料為止。 您的Adobe客戶團隊可以排除日期。 |
