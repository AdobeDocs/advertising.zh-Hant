...
| | [!UICONTROL Google Analytics Login] | 指定用來存取此資料來源資料的登入/電子郵件地址。 登入必須註冊到[!DNL Google]帳戶，並且擁有[!DNL Google Analytics]帳戶的「讀取和分析」許可權。 請參閱[&#x200B; [!DNL Google Analytics]中指派使用者許可權的](https://support.google.com/analytics/answer/9305587)指示。<br><br><b>注意：</b>如果您變更這個登入帳戶的密碼，則所有與該帳戶開啟的連線都會關閉。 若要繼續同步處理資料，請返回此頁面，然後[重新驗證](data-source-reauthenticate.md)。 |
