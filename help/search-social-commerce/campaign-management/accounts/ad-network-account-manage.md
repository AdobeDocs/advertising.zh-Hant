...
此帳戶使用舊版的AMO ID追蹤程式碼格式，可讓Adobe Advertising與Adobe Analytics共用帳戶的相關資料。 此 [最新格式](/help/integrations/analytics/ids.md#amo-id-formats) 包含行銷活動ID和廣告群組ID的引數，這些引數對於在行銷活動和廣告群組層級精確報告是必要的 [!DNL Google Ads] Analytics中的最高成效行銷活動以及草稿和實驗行銷活動：
