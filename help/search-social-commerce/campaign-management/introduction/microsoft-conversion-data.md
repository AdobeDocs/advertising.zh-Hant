...
# [!DNL Microsoft Advertising] 搜尋、社交和Commerce中的轉換資料

搜尋、社交和Commerce會自動同步您追蹤的所有轉換， [[!DNL Microsoft Advertising] 通用事件追蹤(UET)標籤](https://about.ads.microsoft.com/solutions/tools/universal-event-tracking) 用於網站轉換，包括瀏覽轉換，以利報表和最佳化。

所有量度都會自動用於行銷活動管理檢視和基本報表，也可用於產品組合目標以進行最佳化 [!DNL Microsoft Advertising] 行銷活動。

## 可用的轉換資料

搜尋、社交和Commerce同步資料，以找出符合「[!DNL Include in 'Conversions']「選項」已啟用，提取過去35天的資料，然後於09前每日提取資料的變更:00-10:00 （廣告商時區）。 歷史資料可能會每天變更，因為每次點按都會追蹤新的轉換。

各有兩個量度 [[!DNL Microsoft Advertising]-tracked轉換](https://help.ads.microsoft.com/apex/index/3/en-us/n5012) (您設定於 [!DNL Microsoft Advertising])中，會使用在中設定的轉換名稱，自動提供搜尋、社交和Commerce使用 [!DNL Microsoft Advertising]. 每次轉換的量度包括：
...
## 如何比較中的轉換資料 [!DNL Microsoft Advertising] 在搜尋、社交和Commerce中使用資料
