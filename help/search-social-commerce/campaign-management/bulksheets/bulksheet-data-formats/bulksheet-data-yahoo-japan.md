...
| [!UICONTROL AMO ID] | 不適用：建立<br><br>可選：編輯或刪除 | 不適用：建立<br><br>可選：編輯或刪除 | 不適用：建立<br><br>可選：編輯或刪除 | 不適用：建立<br><br>可選：編輯或刪除 | 不適用 | （在產生的Bulksheets中） [!DNL Adobe] — 為同步實體產生的唯一識別碼。 若為回應式搜尋廣告，請 [!UICONTROL AMO ID] 編輯或刪除廣告時需要使用，除非您包含 [!UICONTROL Ad ID]. 若要編輯所有其他實體型別的資料，請使用 [!UICONTROL AMO ID]，則 [!UICONTROL AMO ID] 是編輯或刪除資料的必要專案，除非您包含實體ID和父實體ID。<br><br>搜尋、Social和Commerce會使用值來決定要編輯的正確身分，但不會將ID發佈至廣告網路。 |
