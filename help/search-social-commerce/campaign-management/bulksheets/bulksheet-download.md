...
1. 於 **[!UICONTROL Selections]** 標籤，在欄位中輸入或選取資訊。
...
| [!UICONTROL Filters] | | 要包含在Bulksheet中的特定行銷活動、廣告群組、廣告/創意、關鍵字及/或版位的條件：<ol><li>選取引數（實體名稱或ID；或創意、關鍵字或位置的任何元素），選取運運算元，然後輸入適用的值。<br>例如，只傳回標題中包含「鞋子」的廣告 [!DNL Google Ads] 帳戶，選取 *[!UICONTROL Headline]*，選取 *[!UICONTROL contains]*，然後輸入 `shoes` 在輸入欄位中。</li><li>（若要套用其他篩選器）對於每個其他篩選器，請按一下 **[!UICONTROL +Add Filter]**，選取 **[!UICONTROL AND]** 或 **[!UICONTROL OR]**，選取廣告元素或關鍵字和運運算元，然後輸入適用的值。</li></ul> |
