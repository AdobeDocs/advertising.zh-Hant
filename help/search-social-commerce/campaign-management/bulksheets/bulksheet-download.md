...
您可以使用一個或多個帳戶的自訂設定來建立大量表單 [支援的廣告網路](bulksheet-about.md#bulksheet-functionality-by-network). 大量表單包含搜尋、社交和商務中的資料。
...
| | [!UICONTROL Perform Pre-sync] | （可選）指示Search、Social和Commerce將其檔案與指定的行銷活動同步，以確保所有資料都相同。 依預設，不會選取此選項。<br><b>注意：</b> 搜尋、Social和Commerce會每天自動與廣告網路同步一次，無論何時在廣告網路偵測到新行銷活動，以及每次您從Search、Social和Commerce中變更行銷活動資料時。 如果您認為行銷活動或帳戶資料的最近變更是直接在廣告網路中或使用廣告網路的案頭編輯器，請選取此選項。 選取此選項時，建立大量表單所需的時間會比較長。 |
