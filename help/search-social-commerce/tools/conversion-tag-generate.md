...
## （新UI）在「搜尋」、「社交」和「Commerce」中產生並實作轉換追蹤標籤
...
1. 在主工具列中按一下&#x200B;**[!UICONTROL Conversion Tag]**。

1. 指定[轉換標籤設定](#conversion-tag-settings)。

1. 按一下&#x200B;**[!UICONTROL Generate]**。

1. 按一下「**[!UICONTROL Copy]**」以將標籤複製到剪貼簿。 將標籤提供給廣告商或代理商以進行實作。

>[!NOTE]
>
>新轉換標籤中的每個量度都會自動列在[!UICONTROL Admin] > [!UICONTROL Conversions]中，即使它尚未實作，或其所在的網頁尚未收到任何點按。 此行為不同於手動或其他方式建立之標籤中的量度行為，這些標籤不會列在[!UICONTROL Admin] > [!UICONTROL Conversions]中，直到其所在的其中一個網頁收到點按為止。 但在所有情況下，每個量度一開始都會從投資組合目標、報表和檢視中排除，直到您明確提供它們為止。 將量度新增至投資組合目標之前，請先讓量度可供使用，並將量度新增至報表，以驗證量度何時收到點按。
...
**[!UICONTROL Security]：**&#x200B;您網站的安全性通訊協定建立： *[!UICONTROL Standard]* （使用HTTP的網站）或&#x200B;*[!UICONTROL Secure]* （使用HTTP的網站）。

**[!UICONTROL Properties]：**&#x200B;當一般使用者檢視包含轉換標籤的頁面時，要追蹤的一或多個轉換量度。 若要新增量度至清單，請在&quot;[!UICONTROL Property name]&quot;欄位中輸入量度名稱，然後按一下&#x200B;**[!UICONTROL Add]**。
...
## （舊版UI）在「搜尋」、「社交」和「Commerce」中產生並實作轉換追蹤標籤
...
1. 指定[轉換標籤設定](#conversion-tag-settings-legacy)。
...
### Adobe Advertising轉換標籤設定 {#conversion-tag-settings-legacy}
