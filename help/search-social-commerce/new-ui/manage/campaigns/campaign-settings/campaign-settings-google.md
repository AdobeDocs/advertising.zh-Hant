...
**[!UICONTROL Campaign Type]：**&#x200B;廣告的放置位置，以及行銷活動可能包含的廣告型別：
...
**[!UICONTROL Contains EU Political Ads]：**(適用於以歐盟(EU)受眾為目標的行銷活動)行銷活動是否根據歐盟法規2024/90提供的廣告需求，包含政治廣告： *[!UICONTROL No]*&#x200B;或&#x200B;*[!UICONTROL Yes]*。
...
**[!UICONTROL Google Recommended Budget]：** （選用；適用於具有所有必要設定且僅包含廣告群組的效能上限和搜尋行銷活動）按一下「**[!UICONTROL Show Recommendation]**」以檢視[!DNL Google Ads]建議的預算。 目前，只有關鍵字少於40,000個的行銷活動才符合資格。
...
* *[!UICONTROL Enhanced CPC]：*&#x200B;已棄用。 [!DNL Google Ads]已於2025年開始自動將現有[增強型CPC競標策略](https://support.google.com/google-ads/answer/2464964)變更為手動CPC。

* *[!UICONTROL Manual CPC]* （預設）： （不適用於最高成效行銷活動）使用每次點按成本(CPC)模型。 您可以選擇允許廣告網路變更行銷活動的競標：

  * **[!UICONTROL Enable Enhanced CPC]** （預設為停用）：這與使用「[!UICONTROL Enhanced CPC]」選項相同，該選項已過時。 [!DNL Google Ads]已於2025年開始自動將現有[增強型CPC競標策略](https://support.google.com/google-ads/answer/2464964)變更為手動CPC。
...
* *[!UICONTROL Maximize Conversion Value]：* （搜尋、最高成效和智慧型購物行銷活動）廣告網路（而非Search、Social和Commerce）會最佳化競標，以將轉換價值最大化。 選擇性地輸入&#x200B;**[!UICONTROL Target Return on Ad Spend]** (ROAS)作為百分比。 **注意：**&#x200B;此選項適用於具有行銷活動或廣告群組最佳化之產品組合中的行銷活動，但不具有關鍵字層級最佳化之產品組合。 在包含行銷活動或廣告群組最佳化的產品組合中，搜尋、Social和Commerce會最佳化行銷活動層級或（可用時）廣告群組層級的Target ROAS。

* *[!UICONTROL Maximize Conversions]：* （搜尋、顯示和效能最大化行銷活動）廣告網路（而非Search、Social和Commerce）會最佳化競標，以將轉換最大化。 選擇性地輸入&#x200B;**[!UICONTROL Target CPA]** （每次收購成本）和適用的&#x200B;**[!UICONTROL Portfolio]**&#x200B;競標策略。 **注意：**&#x200B;此選項適用於具有行銷活動或廣告群組最佳化之產品組合中的行銷活動，但不具有關鍵字層級最佳化之產品組合。 在包含行銷活動或廣告群組最佳化的產品組合中，搜尋、Social和Commerce會最佳化行銷活動層級或（可用時）廣告群組層級的Target CPA。

* *[!UICONTROL Target CPA]：* （僅限顯示行銷活動）廣告網路（而非Search、Social和Commerce）會根據選擇性的&#x200B;**[!UICONTROL Target CPA]** （每次贏取成本）來最佳化競標，這是您要為贏取（轉換）支付的30天平均金額。 **注意：**&#x200B;行銷活動位於具有行銷活動或廣告群組最佳化之產品組合（但不是具有關鍵字層級最佳化之產品組合）中，具有除[!UICONTROL Weekly]或[!UICONTROL Google Target CPA]之外的任何支出策略。 在包含行銷活動或廣告群組最佳化的產品組合中，搜尋、Social和Commerce會最佳化行銷活動層級或（可用時）廣告群組層級的Target CPA。
...
* *[!UICONTROL Target Return on Ad Spend]：* （購物行銷活動）廣告網路（而非Search、Social和Commerce）會根據指定的百分比&#x200B;**[!UICONTROL Target ROAS]** （廣告投資報酬率）來最佳化競標。 **注意：**&#x200B;此選項適用於具有行銷活動或廣告群組最佳化之產品組合中的行銷活動（但不是具有關鍵字層級最佳化之產品組合），以及除了[!UICONTROL Weekly]或[!UICONTROL Google Target ROAS]之外的任何支出策略。 在包含行銷活動或廣告群組最佳化的產品組合中，搜尋、Social和Commerce會最佳化行銷活動層級或（可用時）廣告群組層級的Target ROAS。

  使用此競標策略的行銷活動無法使用平均位置和CPC競標資料。

* *[!UICONTROL Viewable CPM]：* （僅限現有、唯讀的[!DNL Gmail]行銷活動）廣告網路（而非Search、Social和Commerce）僅針對測量為可檢視的廣告出價。 **注意：**&#x200B;任何型別的投資組合都不支援此策略的最佳化。

## [!UICONTROL Shopping Settings]索引標籤
...
## [!UICONTROL Advanced Device Options]索引標籤
...
**[!UICONTROL AI Max]：**&#x200B;是否要為行銷活動啟用[[!UICONTROL AI Max]](https://support.google.com/google-ads/answer/15910366) — AI導向的搜尋字詞比對、文字自訂和最終URL擴增。 當您啟用&#x200B;**[!UICONTROL AI Max]**&#x200B;時，會有兩個額外的設定可供使用：

* **[!UICONTROL Text customization]：**&#x200B;是否讓[!DNL Google Ads]使用創作AI，根據現有的廣告和登陸頁面副本來自訂廣告文案、標題和說明。

* **[!UICONTROL Final URL expansion]：**&#x200B;是否讓[!DNL Google Ads]根據搜尋意圖，將流量路由至網站上最相關的登陸頁面。 此設定僅在啟用&#x200B;**[!UICONTROL Text customization]**&#x200B;時可用。

<!-- Clarify why this is "Unspecified" and read-only for me as of 7/23. Also, shouldn't we reword this? -->

**[!UICONTROL Bundling required]：** （僅啟用[!UICONTROL AI Max]功能的現有行銷活動；唯讀）是否必須啟用[!UICONTROL AI Max]才能遵循或修改行銷活動的文字自訂和品牌清單控制項： *[!UICONTROL Required]*、*[!UICONTROL Not required]*&#x200B;或&#x200B;*[!UICONTROL Unspecified]*。

<!-- Is this based on the advanced location options set elsewhere in Google Ads editor? -->

**[!UICONTROL Geo Targeting Type]：** （僅啟用[!UICONTROL AI Max]功能的現有行銷活動；唯讀）當任何行銷活動的廣告群組包含[!UICONTROL Locations of Interest]個目標時，會指出要鎖定的使用者興趣型別： *[!UICONTROL Search Interest]* （對指定位置感興趣的使用者）、*[!UICONTROL Presence]* （對指定位置感興趣或經常感興趣的使用者）或&#x200B;*[!UICONTROL Presence or Interest]* （對指定位置感興趣、經常感興趣的使用者）。
...
**[!UICONTROL Final URL]：**&#x200B;從資產群組建立的所有廣告的最終URL。<!-- For campaigns created within Search, Social, & Commerce, final URL expansion is automatically enabled for the campaign, and [!DNL Google Ads] replaces this value with a more relevant landing page based on the user's search query and intent, and also customizes the headline based on the landing page content. You can disable final URL expansion, or exclude specific URLs from expansion, from within the [!DNL Google Ads] editor. -->

**[!UICONTROL Images]：**&#x200B;廣告的最多15個影像，包括至少一個橫向影像（1.91:1、至少600 x 314畫素）和至少一個正方形影像（1:1、至少300 x 300畫素）。 您可以選擇加入縱向影像（4:5，至少480 x 600畫素）。 請參閱[[!DNL Google Ads] 影像規格](https://support.google.com/google-ads/answer/10724492?hl=en&ref_topic=10631992#zippy=,audience-signal-inputs,video-specifications,image-specifications)。 您可以上傳影像或從[!UICONTROL Asset Library]中選取影像 — 但無法在同一作業中同時選取兩者。
...
**[!UICONTROL Logos]：**&#x200B;至少一個正方形（1:1，至少128 x 128畫素）標誌。 您可以選擇加入橫向（4:1，至少512 x 218畫素）標誌。 您最多可以包含五個圖志。 檢視[[!DNL Google Ads] 標誌規格](https://support.google.com/google-ads/answer/10724492?hl=en&ref_topic=10631992#zippy=,audience-signal-inputs,video-specifications,image-specifications)。 您可以上傳影像或從[!UICONTROL Asset Library]中選取影像 — 但無法在同一作業中同時選取兩者。
...
針對新客戶、現有客戶或兩者最佳化您的行銷活動。 若要使用此設定，您必須先啟用[!DNL Google Ads]帳戶或（如果適用）經理帳戶的新客戶贏取目標。 目標會定義符合資格的現有客戶清單，以及轉換設定中新客戶的其他轉換值。 請參閱[!DNL Google Ads]說明中的步驟1-2 &quot;[啟用新的客戶贏取目標](https://support.google.com/google-ads/answer/14007601)&quot;。

**[!UICONTROL Customer Goal]：**&#x200B;客戶贏取目標型別：

* *[!UICONTROL All Customers]* （預設值）：為新客戶和現有客戶均等地最佳化。

* *[!UICONTROL New Customers]：*&#x200B;優先取得新客戶。 選取此選項會顯示&#x200B;**[!UICONTROL New Customer Bid Adjustment]**&#x200B;設定。

* *[!UICONTROL Existing Customers]：*&#x200B;優先保留現有客戶。

**[!UICONTROL New Customer Bid Adjustment]：** （**[!UICONTROL Customer Goal]**&#x200B;僅設為&#x200B;**[!UICONTROL New Customers]**）鎖定新客戶時，套用至出價的乘數，從0.1到10.0。 預設值為1.0 （無調整）。 例如，1.5倍會針對新客戶將出價提高50%，2.0倍會針對新客戶將出價提高一倍，而0.5倍會針對新客戶將出價降低50%。
