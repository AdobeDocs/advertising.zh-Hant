...
## [!UICONTROL Networks]索引標籤

**[!UICONTROL Networks]：** （搜尋廣告）在廣告群組中放置廣告的方式和位置：

* *[!UICONTROL Only Bing and Yahoo websites]* （預設）：在搜尋網路上刊登廣告競標。

* *[!UICONTROL Only Bing and Yahoo syndicated search partners]：*&#x200B;若要在聯合發行合作夥伴網站上對廣告投標。
...
**[!UICONTROL Gender]：** （選擇性）要包含或排除為目標的特定性別： *[!UICONTROL Male]*、*[!UICONTROL Female]*&#x200B;和&#x200B;*[!UICONTROL Unknown]*。 依預設，會鎖定所有性別。 排除專案一律會覆寫包含專案。
...
**[!UICONTROL Age]：** （選用）要包含或排除為目標的特定年齡類別： *[!UICONTROL 18-24]*、*[!UICONTROL 25-34]*、*[!UICONTROL 35-49]*、*[!UICONTROL 50-64]*、*[!UICONTROL 65+]*&#x200B;和&#x200B;*[!UICONTROL Unknown]*。 依預設，會定位所有年齡。 排除專案一律會覆寫包含專案。
...
**[!UICONTROL Company targets]：** （選擇性）使用者[!DNL LinkedIn]設定檔中的特定公司，包含或排除為目標。 依預設，所有公司都是目標。 若要縮小目標範圍，請搜尋並選取個別企業和中層組織公司。 排除專案一律會覆寫包含專案。

* 若要鎖定所有值，請勿選取任何值。

* 若要包含值，請按一下相鄰的圓圈，以顯示藍色核取記號（![包含](/help/search-social-commerce/assets/include.png "包含")）。 您可以選擇按每個目標公司的指定百分比增加或減少出價。

* 若要排除值，請按兩下相鄰的圓圈，以便出現紅色核取記號（![排除](/help/search-social-commerce/assets/exclude.png "排除")）。

**[!UICONTROL Industry]：** （選擇性）使用者[!DNL LinkedIn]設定檔中的特定產業，以包含或排除為目標。 依預設，會鎖定所有產業。 排除專案一律會覆寫包含專案。
...
**[!UICONTROL Job Function Targets]：** （選擇性）使用者[!DNL LinkedIn]設定檔的特定工作功能，可包含或排除為目標。 依預設，所有工作功能都是鎖定目標。 排除專案一律會覆寫包含專案。
...
（選用）可從廣告群組向客戶提供廣告的次數。 輸入值並選取時間單位(*[!UICONTROL Hour]*、*[!UICONTROL Day]*、*[!UICONTROL Week]*)或&#x200B;*[!UICONTROL Month]*)。
