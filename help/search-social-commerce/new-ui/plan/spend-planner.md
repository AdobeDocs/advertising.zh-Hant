...
1. 在主功能表中，按一下&#x200B;**[!UICONTROL Plan]>[!UICONTROL Spend Planner]**。

1. 使用所選產品組合的目前合併預算檢視資料：

   1. 按一下&#x200B;**[!UICONTROL Select Objective]**。
...
1. （選擇性）若要下載每個產品組合的建議配置和預期收入，請按一下右欄[!UICONTROL Portfolio Allocation]旁的![下載](/help/search-social-commerce/assets/download-spend-recommendation.png "下載")。

   依照瀏覽器的正常程式開啟或儲存檔案。 如需詳細資訊，請參閱瀏覽器的線上說明。

1. （選用）若要檢視使用不同總支出目標的每個產品組合的建議每日支出和預期收入，請在[!UICONTROL Total Spend Target]欄位中輸入所有產品組合的建議每日支出總目標。 然後按&#x200B;**Enter**&#x200B;鍵。
...
## （新UI）套用支出建議 {#spend-recommendations-apply}
...
1. [針對一或多個具有每日預算的產品組合產生支出建議報告](#spend-recommendations-generate)。
...
1. （如果任何預算變更超過20%）在確認訊息中，按一下&#x200B;**[!UICONTROL Confirm]**&#x200B;以核准變更。

## （舊版UI）套用支出建議 {#spend-recommendations-apply-legacy}
...
## （新UI）開啟資料或將資料儲存為[!DNL Microsoft Excel]活頁簿檔案 {#spend-recommendations-download}

您可以開啟或儲存a)折線圖顯示每個成本的成本點和預期收入，以及b)目前和建議媒體組合的環形圖。 [這看起來與Portfolio配置報告相同 — 它們應該有何不同？]

1. [針對選取的投資組合產生支出建議報告](#spend-recommendations-generate)。

1. 在報表上方，按一下![下載](/help/search-social-commerce/assets/download-spend-recommendation.png "下載")。

   依照瀏覽器的正常程式開啟或儲存檔案。 如需詳細資訊，請參閱瀏覽器的線上說明。

## （舊版UI）開啟資料或將資料儲存為[!DNL Microsoft Excel]活頁簿檔案 {#spend-recommendations-download-legacy}
