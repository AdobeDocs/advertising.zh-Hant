...
>[!IMPORTANT]
>
>[!DNL Google Ads]和[!DNL Microsoft Advertising]通用事件追蹤(UET)標籤所追蹤的轉換未重新上傳至廣告網路。 如果您將它們包含在目標中，則必須將它們新增至廣告網路編輯器中的行銷活動目標。
...
傳遞至廣告網路的加權目標為所有收集量度值的總和，但不包括[!DNL Google Ads]或[!DNL Microsoft Advertising]通用事件追蹤(UET)標籤所追蹤的轉換。 此值是使用為廣告商的Search、Social和Commerce帳戶設定的歸因方法來計算。
...
GGL_Lead不包含在計算/上傳中，因為它是[!DNL Google Ads]追蹤的量度。 因此，加權目標值的計算方式為((10 x 25) + (500 x 1) + (50 x 0.5)) = 775。
