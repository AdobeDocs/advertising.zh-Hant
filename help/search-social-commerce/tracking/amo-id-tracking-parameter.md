---
title: AMO ID (s_kwcid)追蹤引數
description: 瞭解用來與Adobe Analytics共用Adobe Advertising資料的追蹤引數。
exl-id: 07b78f0b-624e-4dc2-8588-7cc1a86d3945
feature: Search Tracking
source-git-commit: 546e391745b1469efbcc9c2024dfc193224f0ed0
workflow-type: tm+mt
source-wordcount: '101'
ht-degree: 0%

---

# AMO ID (s_kwcid)追蹤引數

*僅整合Adobe Advertising-Adobe Analytics的廣告商*

Adobe Advertising會使用AMO ID附加引數（也稱為`s_kwcid`引數），與Adobe Analytics共用行銷活動的相關資料，此引數包含廣告頻道和廣告網路專用元素。

如需如何將AMO ID引數新增至您的追蹤URL，以及DSP和Search、Social與Commerce的AMO ID格式的詳細資訊，請參閱「[由 [!DNL Analytics]](/help/integrations/analytics/ids.md#amo-id)使用的Adobe Advertising ID」。

>[!MORELIKETHIS]
>
>* [總覽 [!DNL Analytics for Advertising]](/help/integrations/analytics/overview.md){target="_blank"}
>* [由 [!DNL Analytics]](/help/integrations/analytics/ids.md#amo-id){target="_blank"}使用的Adobe Advertising ID
