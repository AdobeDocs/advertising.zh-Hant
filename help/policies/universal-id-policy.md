...
此通用ID需求原則（「原則」）列出在透過Adobe Advertising的[!DNL Adobe's] On-demand Services （「服務」）散佈的廣告中使用通用ID的需求。 此原則是使用隨選服務，參考納入[!DNL Adobe]與客戶（「您」）之間的合約。 本政策中所使用但此處未定義的專有名詞，其意義將列於該合約中。
...
## 展覽A：UID2被禁止的國家 {#prohibited-countries-uid2}
