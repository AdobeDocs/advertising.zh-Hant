...
| | [!UICONTROL Experiences] | 現在，您可以在廣告體驗設定中指定體驗是否使用決策樹目標定位。 具有決策樹定位的體驗和不具有決策樹定位的體驗的工作流程會不同。 | 請參閱「具有決策樹目標定位的體驗的工作流程」和「沒有決策樹目標定位的體驗的工作流程」。 |
