...
1. 當您[建立](experience-create-targeting.md)或[編輯](experience-edit-targeting.md)體驗時，請開啟創意最佳化設定：
   ...
   1. 將游標置於目標節點下方的建立葉節點上，然後執行以下操作：

      * 對於現有捆綁包，請按一下&#x200B;**[!UICONTROL ...]** > **[!UICONTROL Edit Bundles]**。

      * 對於新包，請按一下&#x200B;**[!UICONTROL ...]** > **[!UICONTROL Assign Bundles]**，然後[分配包](experience-assign-creative-bundles.md)。
...
1. 選取關聯套件組合中廣告變體的創意輪換型別：
   ...
   * *[!UICONTROL Weighted]：*&#x200B;根據相對權重，在關聯的創意組合中顯示廣告變體。 以百分比形式輸入每個束的重量。 要對所有關聯的束應用相等的權重，請按一下（![應用相等的權重](/help/creative/assets/apply-equal-weight.png "應用相等的權重")）。 所有選定捆綁包的權重最多必須加100.<!-- For example, if Bundle 1 is 60 and Bundle 2 is 40, then Bundle 1 is shown 60% of the time, and Bundle 2 is shown 40% of the time. -->

   * *[!UICONTROL Algorithmic]:*&#x200B;根據指定的目標更頻繁地顯示最有效的廣告變體。

      * 對於&#x200B;**[!UICONTROL Optimization Goal]**，選擇&#x200B;*[!UICONTROL Click Through Rate]*、（標準視頻廣告體驗）*[!UICONTROL Completion Rate]*&#x200B;或&#x200B;*[!UICONTROL Custom Objective]*。  如果選擇&#x200B;*[!UICONTROL Custom Objective]*，則選擇現有的[廣告DSP自定義目標](/help/dsp/optimization/custom-goal.md)。
...
1. 在[建立](experience-create-targeting.md)或[編輯](experience-edit-targeting.md)體驗時，請開啟創作優化設定：
...
1. 對於第一個計畫：
   ...
   1. 在左欄中，選中要添加到第一個計畫的每個創作包旁邊的複選框。

   1. 指定計畫的開始日期和開始時間以及結束日期和結束時間。
   ...
   1. 選擇建立旋轉類型：
      ...
      * *[!UICONTROL Sequencing]：*&#x200B;以指定的順序旋轉關聯的創意組合（先提供組合1，再提供組合2，依此類推），並指定各組合順序的曝光總數。 提供的廣告大小由可用詳細目錄決定。 可以將序列中的最終束配置為a\)無限期地顯示（預設）或b\)回到第一個束。 例如，您可以在Bundle 1中顯示任何創意以獲得三(3)個印象，然後在Bundle 2中顯示任何創意以獲得一(1)個印象，然後在Bundle 3中顯示任何創意以獲得兩(2)個印象，然後重新開始循環。 或者，一旦顯示了捆綁3中的創意，您可以繼續無限期地顯示捆綁3中的創意，而不是建立循環。 啟用排序時：
...
>[!MORELIKETHIS]
>
>* [為體驗中的最終節點分配和取消分配創作捆綁包](/help/creative/experiences/experience-assign-creative-bundles.md)
