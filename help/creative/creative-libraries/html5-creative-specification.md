---
title: HTML5創意規格
description: 參考HTML5Advertising Creative的創意規格。
...
# HTML5Advertising Creative的創意規格

本檔案概述[!DNL Creative]中HTML5創意專案的需求和API支援。 此API允許開發HTML5創意，其屬性可在創意交付時設定。
...
* **彈性HTML5：**&#x200B;支援最多5個可在創意建立和販運期間設定的登陸頁面URL，也可在創意建立和販運期間修改創意屬性。
...
* 主要HTML檔案可以命名為任何名稱，只要它不包含特殊字元即可，不過建議使用`index.html`。

* 呈現最終創意所需的所有支援資產必須位於與HTML顯示檔案相同的資料夾或主資料夾的子資料夾中。

* 請勿在創意中加入未參考該創意的任何檔案。

### 納入Advertising CreativeJavaScript檔案

主要HTML檔案 — 沒有其他檔案 — 必須包含對JavaScript檔案`AMOLibrary.js`的參照。 使用以下位址呼叫`<head>`區段第一行中的檔案：
...
註冊點進URL和用於參考每個URL的相關引數（稱為`clickTag`）。 這會通知[!DNL Creative]廣告伺服器新增點選追蹤的位置。 您可以使用此API來註冊最多五個點按標籤變數，每個變數都有對應的登陸頁面URL。

>[!NOTE]
>
>您在HTML5創意內容中包含的靜態URL僅用於本機測試目的，將被覆寫。 當您上傳HTML5創意內容時，將會為每個`clickTag`變數定義預設登陸頁面。 當您將上傳的HTML5創意內容指派給廣告體驗時，您可以選擇覆寫每個`clickTag`變數的預設登陸頁面，並在您儲存體驗時[!DNL Creative]將點選追蹤新增到URL。
...
查詢創意屬性變數名稱和值的JSON物件。 物件索引鍵將是屬性名稱，而值將是這些屬性的值。
...
* 登入頁面網域及其子網域不得超過35個字元。 **注意：**&#x200B;最終登陸頁面URL是在DSP中定義，而不是在HTML5資產本身中定義。
...
### 靜態HTML5創意內容的範例HTML檔案(index.html)
