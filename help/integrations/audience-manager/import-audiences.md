...
若要存取DSP或[!DNL Adobe]中的[!DNL Creative]個對象，您必須將對象匯入DSP。 若要存取[!DNL Adobe]中的[!DNL Search, Social, & Commerce]個對象，您必須將對象匯入[!DNL Search, Social, & Commerce]。
...
* （具有[!DNL Analytics]的廣告商）廣告商必須[使用 [!DNL Analytics] `appMeasurement.js` 1.6.4或更新版本實作](https://experienceleague.adobe.com/zh-hant/docs/analytics/implementation/js/overview)。
...
* [位置設定](/help/dsp/campaign-management/placements/placement-settings.md#audience-targeting)：在[!UICONTROL Adobe Segments]區段的[!UICONTROL Audience Targeting]索引標籤上。
...
在[!DNL Search, Social, & Commerce]中，當您從[!DNL Google] > [!UICONTROL Data Source] > [!UICONTROL Adobe Audience]使用[!UICONTROL Campaigns] &quot;[!UICONTROL Audiences]&quot;建立[!UICONTROL Library]對象時，可以使用區段。
