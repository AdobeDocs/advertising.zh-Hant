---
title: 匯入Adobe Audience Manager區段以進行廣告目標定位
...
# 匯入Adobe Audience Manager區段以進行廣告目標定位
...
* 發佈至Adobe CX Enterprise的Adobe Analytics區段
...
* 廣告商必須實作[該 [!DNL Adobe CX Enterprise Identity (ECID) Service]](https://experienceleague.adobe.com/zh-hant/docs/id-service/using/intro/overview) 2.0版或更新版本。 [!DNL Identity Service]提供永續性的通用ID，可識別CX Enterprise所有解決方案的訪客。

  實作包括將[!DNL Identity service]程式碼新增至廣告商網站上的每個網頁。

* 組織必須已啟用CX Enterprise服務[&#128279;](https://experienceleague.adobe.com/zh-hant/docs/core-services/interface/services/overview)，且具有CX Enterprise [!DNL Organization ID] （先前稱為[!DNL IMS org ID]）。

  [!UICONTROL Organization ID]可讓擁有多個Adobe CX Enterprise產品的組織在某些產品之間共用資料。
...
* 將兩個目的地對應至所有Audience Manager區段，讓Audience Manager可以與DSP廣告商帳戶（與用於Audience Manager的相同CX Enterprise [!DNL Organization ID]相關聯）共用區段。
...
1. 在[!DNL Search, Social, & Commerce]內，[!DNL Search, Social, & Commerce]實作團隊或具有直接存取使用者端管理員角色的使用者應該從[!UICONTROL Search, Social, & Commerce] > [!UICONTROL Admin] > [!UICONTROL Audience Manager Setup]起始匯入。
   ...
   需要組織的Adobe CX Enterprise [!DNL Organization ID] ([!DNL IMS org ID])。 此ID必須與用於組織Audience Manager帳戶的ID相同。
