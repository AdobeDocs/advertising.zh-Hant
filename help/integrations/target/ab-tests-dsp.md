...
當您將DSP巨集新增至點進URL （使用者點按廣告並到達登陸頁面時所顯示的URL）時，DSP會在點進URL中加入`${TM_PLACEMENT_ID}`，自動擷取位置索引鍵。 此巨集會擷取英數字元版位索引鍵，而非數值版位ID。
...
1. 從DSP曝光資料設定Audience Manager區段：
   ...
   1. [為Audience Manager中的區段建立建立建立規則型特徵](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/traits/trait-builder/create-onboarded-rule-based-traits.html)。
   ...
   1. [為Audience Manager中的新特徵設定測試區段](https://experienceleague.adobe.com/docs/audience-manager/user-guide/features/segments/segment-builder.html)，選取`Ad Cloud`做為&#x200B;**[!UICONTROL Data Source]**。
...
* 在工作區中建立面板，專用於執行測試的Adobe Advertising促銷活動、套件或位置。 使用摘要視覺效果顯示同一報表中的Adobe Advertising度量作為[!DNL Target]測試效能。
