...
由於[!DNL Analytics]資料會傳送至Adobe Advertising以進行報告和最佳化，因此資料受限於針對Adobe Advertising中的廣告商設定的歸因規則，包括曝光和點按回顧期間。

在Adobe Advertising中![廣告商層級的回顧期間設定](/help/integrations/assets/a4adc-lookbacks.png)
...
* Adobe Advertising歸因曝光回顧期間：廣告曝光發生後，該曝光會歸因於轉換的天數。 預設值為14天，最大值為30天
  ...
  >[!NOTE]
  >
  > 印象回顧期間是Adobe Advertising專屬的，而非[!DNL Analytics for Advertising]報告。

[!DNL Analytics for Advertising] JavaScript會使用這些設定來判斷多久以前會將檢視專案或網站的點進專案視為有效。 如需如何決定閱覽和點進的詳細資訊，請參閱「[Analytics使用的Adobe Advertising ID](ids.md)」。

## [!DNL Analytics]中的Adobe Advertising資料

[!DNL Analytics]在Analytics點選中設定Adobe Advertising ID (AMO ID)，受限於廣告商的[!DNL eVar]持續性設定，此設定同時適用於點進和檢視。 持續性設定是在Adobe Advertising後端設定，您的Adobe客戶團隊可以加以變更。
...
為網站實作Experience Cloud Identity Service後，包含[!DNL Analytics]或Adobe Advertising資料的點選會包含補充ID。
