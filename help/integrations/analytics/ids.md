...
*適用於Advertising Creative、Advertising DSP和Advertising Search、Social和Commerce*
...
在[!DNL Analytics]報表中，您可以搜尋[!UICONTROL AMO ID]維度並使用[!UICONTROL AMO ID Instances]量度來尋找AMO ID資料。 [!UICONTROL AMO ID]維度內含所有擷取的AMO ID值，而[!UICONTROL AMO ID Instances]量度則表示網站擷取AMO ID值的頻率。 例如，如果同一個搜尋廣告被點按四次，但[!DNL Analytics]追蹤了七個網站專案，則[!UICONTROL AMO ID Instances]將是七(7)，[!UICONTROL Clicks]將是四(4)。

對於[!DNL Analytics]內的任何報告或稽核，最佳實務是使用AMO ID及其對應的執行個體。 如需詳細資訊，請參閱「在[!DNL Analytics]和Adobe Advertising之間的預期資料差異」中的[針對 [!DNL Analytics for Advertising]](data-variances.md#data-validation)的點進資料驗證。
